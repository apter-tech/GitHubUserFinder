...
fun FavouriteGitHubUsersContent(
    ...
    Scaffold(topBar = {
        CommonAppBar(
            title = stringResource(MR.strings.favourite_screen_title),
            onArrowClick = onArrowClick,
            trailing = {
                if (state.data.isNotEmpty()) {
                    Icon(
                        painter = painterResource(imageResource = MR.images.ic_trash),
                        contentDescription = stringResource(MR.strings.clear_button_content_description),
                        modifier = Modifier
